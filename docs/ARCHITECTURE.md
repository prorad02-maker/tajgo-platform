...
TajGo — сервис доставки для Худжанда. Один UID сохраняет клиентский доступ и
после проверки может получить дополнительный курьерский режим. В приложении три интерфейса:
...
- **Администратор** — проверяет заявки курьеров, управляет доступом и видит журнал решений.

Бэкенд — Firebase: основной вход по телефону (Auth), debug-only анонимный
демо-вход, данные и realtime-обновления (Firestore).
Карта — flutter_map (OpenStreetMap), без Google-зависимостей.

Курьерский доступ не выдаётся выбором роли. Пользователь проходит 5-шаговую
анкету в `courier_applications/{uid}`; только транзакционное решение администратора
добавляет роль courier и создаёт private/public профиль. После approve выполняется
одноразовый onboarding, затем разрешается выход online.
...
lib/
  ...
  core/                      # общая логика без UI
    ...
    services/                # доступ к Firebase
      auth_service.dart      # Phone Auth + безопасный anonymous link
      user_repository.dart   # users, профиль и последний режим
      account_mode_service.dart
      account_migration_service.dart
  ...
  features/                  # экраны, сгруппированные по фичам
    splash/                  # короткий брендовый splash
    startup/                 # intent + App Startup Router
    auth/                    # телефон, SMS, профиль, конфликт аккаунтов
    account/                 # общий профиль и настройки
...
```text
SplashScreen → AppStartupRouter
  ├─ нет Firebase user → IntentSelectionScreen → PhoneAuthScreen
  ├─ auth есть, профиль не заполнен → ProfileCompletionScreen
  ├─ lastMode=customer → CustomerHomeScreen
  └─ lastMode=courier + approved → CourierHomeScreen
```

Первый экран спрашивает намерение («Заказать доставку» / «Зарабатывать с
TajGo»), но не выдаёт роль. Клиентский режим есть у каждого. Курьерский режим
доступен только при `courierStatus=approved`; переключение использует тот же UID
и не требует повторного SMS. Старый `RoleScreen` удалён.
...
|---|---|---|
| uid | string | = Auth uid |
| displayName / name | string | имя + legacy-зеркало |
| phoneNumber | string? | подтверждённый телефон Firebase Auth |
| photoUrl | string? | фото профиля |
| roles | array | всегда `customer`, после одобрения также `courier` |
| lastMode | string | `customer` \| `courier` |
| courierStatus | string | `none/draft/pending/approved/rejected/suspended` |
| phoneVerified | bool | номер подтверждён не debug-anonymous входом |
| profileComplete | bool | минимальный профиль заполнен |
| role | string | legacy-зеркало режима; `admin` остаётся полномочием |
| city | string | город (пока `Худжанд`) |
| createdAt / updatedAt | timestamp | серверное время |
