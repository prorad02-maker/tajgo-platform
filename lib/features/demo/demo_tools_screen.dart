...
class _DemoToolsScreenState extends State<DemoToolsScreen> {
  ...
  Future<void> _seedMarketplace() async {
    await TajGoScope.of(
      context,
    ).marketplaceRepository.publishSampleCatalog(adminId: _uid);
  }
            ...
            builder: (context, orderSnapshot) {
              ...
              return ListView(
                ...
                children: [
                  ...
                  const SizedBox(height: 14),
                  _ToolButton(
                    label: 'Тест · Опубликовать demo marketplace',
                    busy: _busy,
                    onPressed: () => _confirm(
                      'Создать тестовую витрину?',
                      'Будут созданы 6 партнёров и 18 товаров. Повторный запуск безопасно обновит их.',
                      _seedMarketplace,
                    ),
                  ),
                  _ToolButton(
